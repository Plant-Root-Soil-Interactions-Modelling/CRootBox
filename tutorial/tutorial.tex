...
\item[10-16] Definition of a rotated rhizotron: L12 creates the flat container with a small height, this container is then rotated and translated into the desired position. 
...
Its possible to simulate multiple root systems. To speed up computation the simulation is performed in parallel.
The following example shows how simple this can be achieved in Python, by a small plot scale simulation.
...
L25 performs the parallel computation. Nothing is returned by the simulate function, but the root system instances are updated.

\item[27-37] Saves each root systems, and additionally, saves all root systems into a single file. 
