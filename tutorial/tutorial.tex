...
This is the function that is called, when the tropisms are created from the parameters ($tt$, $N$, $sigma$). This is necessary when user defined tropism are created from a parameter file, where $tt$ is the number of the new tropism type. 
...
If the whole concept of the random optimization is altered, Tropism::getUCHeading must be overwritten. If the geometry model is also changed Tropism::getHeading.

The following example shows how to implement a new tropism in Python. Two new tropism are introduced:
The first does nothing put outputs the incoming arguments of the method Tropism::tropismObjective to the command line.  
The second one, is a Plagiotropism that changes with time to Gravitropism.
...
\item[16-32] Again, we extend the class from Tropism::tropismObjective.
...
\lstinputlisting[firstline=1,lastline=50, language=Python, caption=Example 4c]{../example4c.py}
...
\lstinputlisting[firstline=51,lastline=81, language=Python, caption=Example 4c]{../example4c.py}
