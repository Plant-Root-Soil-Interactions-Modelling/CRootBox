...
public:
	...
	enum ScalarTypes { st_type = 0, st_radius = 1, st_order = 2, st_rootID = 21, st_time = 3, st_length = 4, st_surface = 5, st_one = 6, st_volume = 19,
    ...
    std::vector<Vector3d> getNodesDGF() const; ///< Copies all root system nodes into a vector
    ...
    std::vector<Vector2i> getSegmentsDGF() const; ///< Copies all segments indices into a vector
	std::vector<double> getLengthFromOrigin() const; ///< Calculate distance of a segment from the branch's origin
	...
	void writeGeometry(std::ostream & os) const; ///< writes the current confining geometry (e.g. a plant container) as paraview python scrip
	void writeDGF(std::ostream & os) const;
...
private:
	...
	int getNodeIndex() { nid++; return ((nid<=1) ? nid : std::max(nid-rsparam.maxB,1)); } ///< returns next unique node id, called by Root::addNode()
