 ...
 */
Root::Root(const Root& r, RootSystem& rs) :rootsystem(&rs), param(r.param), iheading(r.iheading), id(r.id), parent_base_length(r.parent_base_length), parent_ni(r.parent_ni), alive(r.alive),
    ...
    if (alive) { // dead roots wont grow
        ...
        if (age>0) {
            ...
            if (active) {
                ...
                // length increment
                double age_ = getAge(length); // age>=age_ for impeded growth
                double targetlength = getLength(age_+dt);
                ...
                double dl = std::max(scale*e, 0.); // length increment, dt is not used anymore
