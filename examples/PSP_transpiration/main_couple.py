...
pot_trans = 2*np.array([-1.15741e-10]) # # m^3 s^-1 potential transpiration
top_pot = -1500 # J kg^-1 top potential (wilting point)

#
# Simulation parameters
#                   
simTime = 28*24*3600 # s
maxTimeStep = 3600           
dt = 36.              
time = 0              
...
out_next = out_delay             
out_img = np.zeros((n,round(simTime/out_delay)))
out_c = 0
...
while (time < simTime):
    ...
    while (not success):
        ...
        for i in range(n+2): # reset to old time step
    ...
    soil_p2 = lambda x,y,z : soil_p(x,y,z, vap.psi, soil.lowerDepth) # J/kg
    ...
    if dirichlet or (x[0]<top_pot):        
        ...
        print("using dirichlet (eff Trans = "+str(eff_trans)+" m^3 s^-1)")
    ...
    for i in range(0,len(seg)): # calculate sink
        ...
        z2 = nodes[seg[i,1],2]
        z = 0.5*(z1+z2)
        if z>-soil.lowerDepth: # -1 meter
            ind = math.floor(-z/soil.lowerDepth*vap.n)
            sink[ind] += radial_flux[i] * dt       
    vap.plant.t[1:n+1] = sink   
    ...
    print("simtime =", format(time/3600./24., '.3f'), "days, spent ="+ s0 +"s ("+s1+", "+s2+", "+s3+", "+s4+")", 
          rs.getNumberOfNodes(), "nodes,", "dt =", dt, " iter =", int(nrIterations), "cum trans", ctflux, "trans",-float(np.sum(sink)), "Evapotranspiration:", format(sumETr, '.3f') )
    ...
    if time>=rs_out_next: 
        ...
        ax1 = fig1a.gca()
        ax2 = ax1.twiny()          
        rsl = v2a(rs_ana.distribution(rb.ScalarType.length,0.,100.,n,False))
#         print(rsl[0:10])
#         print(sink[0:10])
#         print("press key")
#         input()
        ...
        l2 = ax2.plot(rsl,-vap.z[1:len(vap.z)-1],color='green') # for greyscale...     
        ...
        l1 = ax1.plot(-sink,-vap.z[1:len(vap.z)-1],'-',color='blue')   
    ...
    if time>=out_next:
        ...
        out_img[:,out_c] =  vap.theta[1:n+1]# plot effective saturation ,/soil.thetaS             
        out_c += 1       
        fig1.clf()
        ax = fig1.gca()
